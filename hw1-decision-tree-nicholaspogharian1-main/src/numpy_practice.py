...
def replace_nonfinite_in_place(x):
    ...
    """
    np.put(x, np.where(~np.isfinite(x)), 0)
...
def replace_nans_out_of_place(x):
    ...
    """
    return np.insert(np.delete(x, np.where(np.isnan(x))), np.array(np.where(np.isnan(x)))[0], 0)
...
def find_mode(x):
    ...
    Returns:
        the mode of x
    """
    return np.argmax(np.bincount(x.astype(int)))
...
def flip_and_slice_matrix(x):
    ...
    Returns:
      a numpy matrix
    """
    return np.flip(x, axis=1)[0:len(x):3,:]
