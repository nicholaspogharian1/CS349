...
def part_a_plot():
    ...
    fig.suptitle("Effects of Sample Size and Number of Nearest Neighbors in KNN Regression")
    ...
    for idx, amount in enumerate(amounts):
        ...
        #gather our data
        testdata = load_frq_data(amount)
        train_mse = np.zeros(4)
        test_mse = np.zeros(4)
        title='literally whatever' #we arent plotting anything with visualize_model so this doesnt matter
        for i in range(4):
            model=KNearestNeighbor(x_axis[i],aggregator="mean")
            train_mse[i], test_mse[i] = visualize_model(testdata, model, title)
    ...
    plt.savefig("free_response/plot_knn.png")
...
def polynomial_regression_experiment():
    ...
    for amount in amounts:
        for degree in degrees:
            ...
            visualize_model(testdata, model, title)
