...
class ReluActivation:
    ...
    def backward(self, grad, lr=None):
        ...
        new_grad = np.greater(self.input_, 0).astype(int)
        return new_grad * grad
...
class FullyConnected:
    ...
    def backward(self, grad, lr=0.1):
        ...
        if self.regularizer is not None:
            #if l1 regularization use l1 grad
            if self.regularizer.penalty=="l1":
                self.weights -= lr * self.regularizer.l1_grad(self.weights)
            #if l2 regularization use l2 grad
            if self.regularizer.penalty=="l2":
                self.weights -= lr * self.regularizer.l2_grad(self.weights)
